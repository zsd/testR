 ...
 * url：项目路径
 * allDiv：总的Div的Id
 * authority：权限信息（type为按钮类别：add、delete、export、change这4种，权限值为value：true或者false）
...
//查询路径
Paging.prototype.url = "";
//当前页数
Paging.prototype.pageNoId = "";
//一页显示个数
Paging.prototype.pageSizeId = "";
//选择的字符串
Paging.prototype.itemsName = "";
//查询条件
Paging.prototype.queryCriteria ={};
//全选按钮Id
Paging.prototype.selectAllId = "";
//删除按钮Id
Paging.prototype.deleteId = "";
//导出按钮Id
Paging.prototype.exportId = "";
//新增按钮Id
Paging.prototype.addId = "";
//单选框唯一样式识别
Paging.prototype.rowIdImputClass = "";
//表格详情
Paging.prototype.tableInfo = [];
//唯一标识
Paging.prototype.onlyOne = "";
//新增按钮权限
Paging.prototype.addAuthority = false;
//删除按钮权限
Paging.prototype.deleteAuthority = false;
//编辑按钮权限
Paging.prototype.changeAuthority = false;
//导出按钮权限
Paging.prototype.exportAuthority = false;

//时间格式化
Paging.prototype.formatDate = function (strTime) {
...
//计算两个时间差
Paging.prototype.dateDiff = function(sDate1, sDate2){
...
//分页
Paging.prototype.gotoPage = function (newPageNo) {
...
//分页
Paging.prototype.getPage= function (pageNo, totalPages) {
    var page1 = "";
    if (totalPages <= 3) {
...
//获取已选择的ID字符串
Paging.prototype.getSelectId = function (){
    var ids = '';
...
//添加分页信息
Paging.prototype.addSelectUser = function (page){
...
//查询分页数据并显示到页面
Paging.prototype.searchData = function (url,data){
    var me = this;
    $.ajax( {
        dataType: "json",
        type: "post",
            ...
            try {
                ...
                for (var i=0;i<result.length;i++) {
                    ...
                    if(me.changeAuthority){
                        changeStr = '<td><button type="button" onclick="showDetail(\''+result[i].id+'\')" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>&nbsp;编辑</button></td>';
                    }
                    //遍历传入的表格列，并构建成表格内容格式
...
//初始化页面Div
Paging.prototype.initDiv = function (initPagm){
    var tableTh = '';
    var addButton = '';
    var deleteButton = '';
    var exportButton = '';
...
//点击新增按钮方法（实例化后需要重写）
Paging.prototype.addFn = function(){};
//点击导出按钮方法（实例化后需要重写）
Paging.prototype.exportXml = function(){};
//权限处理方法
Paging.prototype.authorityHandle = function(initPagm){
    //权限处理
