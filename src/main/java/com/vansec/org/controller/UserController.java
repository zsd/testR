...
public class UserController {
    ...
    public ModelAndView gotoPage() {
        ...
        //添加系统日记
        logService.add(LogFactory.insert(Log.TYPE_GETBYID,"访问用户管理页面","机构管理", SecurityContextHolder.getPost().getUser()));
        ...
        try {
            ...
            if(count == 0){
                if (StringUtils.isNotBlank(user.getId())) {
                    ...
                    //添加系统日记
                    logService.add(LogFactory.insert(Log.TYPE_MOD,"修改用户","机构管理", SecurityContextHolder.getPost().getUser()));
                } else {
                    userService.save(user);
                    //添加系统日记
                    logService.add(LogFactory.insert(Log.TYPE_ADD,"添加用户","机构管理", SecurityContextHolder.getPost().getUser()));
                }
    ...
    public Map<String, Object> delete(String idStr) {
        ...
        try {
            ...
            //添加系统日记
            logService.add(LogFactory.insert(Log.TYPE_DEL,"删除用户","机构管理", SecurityContextHolder.getPost().getUser()));
     ...
     * 分页.
     */
    @RequestMapping(value = "getAllUser", produces = "text/plain; charset=UTF-8")
    @ResponseBody
    public String search(@RequestParam(required = false) String name ) {
        List<Map<String, Object>> list = userService.getAllUser(name,null);
